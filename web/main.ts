...
class GitGraphView {
	...
	constructor(viewElem: HTMLElement, prevState: WebViewState | null) {
		...
		if (prevState && !prevState.currentRepoLoading && typeof this.gitRepos[prevState.currentRepo] !== 'undefined') {
			this.branches = prevState.forkBranches;
			this.branchFilterElem.value = prevState.branchFilter;
	...
	private loadRepoInfo(branchOptions: ReadonlyArray<string>, branchHead: string | null, remotes: ReadonlyArray<string>, stashes: ReadonlyArray<GG.GitStash>, isRepo: boolean) {
		...
		this.branches = this.branches?.length === 0
			? (this.currentBranches.includes(SHOW_ALL_BRANCHES)
...

		{
			...
			const recursiveRender = (node: ForkUiBranch, container: HTMLElement) => {
				...
				if (isLeaf) {
					...
					hideBranchBtn.classList.toggle('active', this.branches.find(b => b.name === node.fullName)?.visibility === ForkBranchVisibility.Hidden);
	...
	private renderTable() {
		...
		for (let i = 0; i < this.commits.length; i++) {
			...
			let message = '<span class="text d-block" tabindex="0">' + textFormatter.format(commit.message) + '</span><input class="inline-input d-none" type="text" value="' + textFormatter.format(commit.message) + '">';
	...
	private rebaseAction(obj: string, name: string, actionOn: GG.RebaseActionOn, target: DialogTarget & (CommitTarget | RefTarget)) {
		dialog.showForm('Are you sure you want to rebase ' + (this.gitBranchHead !== null ? '<b><i>' + escapeHtml(this.gitBranchHead) + '</i></b> (the current branch)' : 'the current branch') + ' on ' + actionOn.toLowerCase() + ' <b><i>' + escapeHtml(name) + '</i></b>?', [
			...
			{ type: DialogInputType.Checkbox, name: 'Override author\'s date', value: this.config.dialogDefaults.rebase.ignoreDate, info: 'Only applicable to a non-interactive rebase.' }
		], 'Yes, rebase', (values) => {
	...
	private observeTableEvents() {
		...
		this.branchFilterElem.addEventListener('input', (_: Event) => this.applyBranchTreeFilter());
